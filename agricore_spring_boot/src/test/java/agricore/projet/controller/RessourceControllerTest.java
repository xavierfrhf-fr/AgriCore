...
public class RessourceControllerTest {
    ...
    void shouldCreateAndAttributesOk() throws Exception {
        // given
        RessourceResponseDTO response = new RessourceResponseDTO(RESSOURCE_ID, RESSOURCE_NOM, RESSOURCE_QUANTITE,
                RESSOURCE_PRIX, RESSOURCE_STOCK_MIN, ZONE_ZONE_ID, ZONE_ZONE_NOM);
    ...
    void shouldUpdateAndAttributesOk() throws Exception {
        ...
        Mockito.when(this.ressourceService.update(Mockito.eq(RESSOURCE_ID), Mockito.any())).thenReturn(response);
